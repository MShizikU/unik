...
foreach($contactsData as $key => $contact) {
    ...
    $data = array(
        ...
        "created_at" => ($contact->created_at != null) ?DateTime::createFromFormat('U',$contact->created_at)->setTimezone(new DateTimeZone('Europe/Moscow'))->format('Y-m-d H:i:s') : null,
        "updated_at" => ($contact->updated_at != null) ? DateTime::createFromFormat('U',$contact->updated_at)->setTimezone(new DateTimeZone('Europe/Moscow'))->format('Y-m-d H:i:s') : null,
