<?php
function execSaveInBd($table_name, $data, $log_file, $database, $checker_keys){

    $checker_string = "";
    foreach($checker_keys as $key => $value){
        $checker_string = $checker_string . "" . $value . " = '" . $data[$value] . "' AND ";
    }
    $checker_string = substr($checker_string, 0, strlen($checker_string) - 5);
    $checkIfExistSQL = 'SELECT id FROM ' . $table_name .' WHERE ' . $checker_string;
