...
for count in range(1,counter+1):
    ...
    if (current_diferience <= 28) and (current_diferience>0):
        ...
        if (current_diferience == 1):
            ...
            bot.send_message(user_id,'1. https://telegra.ph/Den-5-Kak-lechitsya-tuberkulez-10-10 ')
            bot.send_message(user_id,'2. Брошюра «Короткие курсы лечения ТБ» http://tbicvladimir.org/index.php/polezno-znat-2/157-kak-ne-dat-prosnutsya-tuberkulezu-v-vashem-organizme-2')
            bot.send_message(user_id,'1. Цель лечения, концентрация препаратов, формирование ЛУ \nhttps://youtu.be/8LC5ZoKvCFE')
            bot.send_message(user_id,'2. Лечение туберкулеза https://youtu.be/lOD2d7KX63c\n')
