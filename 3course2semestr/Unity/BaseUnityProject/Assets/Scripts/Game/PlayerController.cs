...
public class PlayerController : MonoBehaviour {
    ...
    void JumpHandler()
    {
        ...
        if (jAxis > 0f)
        {
            ...
            {
                JumpsCount += 1;
                TextMeshProUGUI textElement = _hudPanel.GetComponentInChildren<TextMeshProUGUI>();

                if (textElement != null)
                {
                    textElement.text = "Jumps: " + JumpsCount;
                }
                else
                {
                    Debug.LogWarning("TextMeshProUGUI component not found in the panel.");
                }
                pressedJump = true;
    ...
    public void GenerateSpheres()
    {
        ...
        {
            Vector3 position = playerPosition + UnityEngine.Random.insideUnitSphere * 5f;
            float size = UnityEngine.Random.Range(sphereSizeMin, sphereSizeMax);
