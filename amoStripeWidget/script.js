define(['jquery', 'underscore'], function($, _) {
	...
	function renderTable(data, mainBlock, paidOnlyCheckBoxStatus = 0, addedOnlyCheckboxStatus = 0) {
		...
		generateCalendar("alfaamo_payment_report_calendar", "alfaamo_payment_report_calendar", filterContainer);
		filterContainer.prepend(paidOnlyDiv, addedOnlyDiv);
		mainBlock.prepend(filterContainer);
		inputMasksEventListner();
		filterCalendar = new CustomCalendar(document.querySelector("#alfaamo_payment_report_calendar"), (cal) => console.log(cal), (cal) => console.log(cal));
	...
	function generateCalendar(id, name, parent){
		...
		const inputSecond = $("<input>").attr({
			...
			"id": id + "_second_input",
			"placeholder": "до " + '_.__.____',
			"data-slots": ""
		});
	
		const clearIcon = $("<div>").addClass("cal_input_clear");
		const svgClear = $("<svg>").attr({
			"xmlns": "http://www.w3.org/2000/svg",
			"width": "14",
			"height": "14",
			"viewBox": "0 0 14 14",
		...
		const pathClear = $("<path>").attr({
			"d": "M12.8337 2.34175L11.6587 1.16675L7.00033 5.82508L2.34199 1.16675L1.16699 2.34175L5.82533 7.00008L1.16699 11.6584L2.34199 12.8334L7.00033 8.17508L11.6587 12.8334L12.8337 11.6584L8.17533 7.00008L12.8337 2.34175Z",
			"fill": "#AFBCBE"
		});
		...
		const pathBack = $("<path>").attr({
			"d": "M1.90039 5.5L7.30039 0.303848L7.30039 10.6962L1.90039 5.5Z",
			"fill": "#AFBCBE"
		});
		...
		const pathFront = $("<path>").attr({
			"d": "M8.59961 5.5L3.19961 10.6962L3.19961 0.303848L8.59961 5.5Z",
			"fill": "#AFBCBE"
		});
	...
	class CustomCalendar{
		  ...
		  if (input.value.replace(/\_/g, "").replace(/\D+/g, "").replace(/\./g, "").replace(" ", "").length === 8){
			...
			if (!isNaN(date)){
			  if (date.getTime() === this.firstSelectedDay){
				if (isFirst){
				  input.value = "от " + this.getDateStringFromTimestamp(this.firstSelectedDay);
				}else{
				  input.value = "до " + this.getDateStringFromTimestamp(this.secondSelectedDay);
				}
				return;
			  ...
			  if (isFirst && (date < this.secondSelectedDay || this.secondSelectedDay === null)){
				...
				this.secondInput.value = "до " + this.getDateStringFromTimestamp(this.firstSelectedDay);
				this.firstSelectedDay = date;
				this.firstInput.value = "от " + this.getDateStringFromTimestamp(date);
			  }
			  else if (isFirst && date > this.secondSelectedDay)
			  {
				this.firstInput.value = "от " + this.getDateStringFromTimestamp(this.secondSelectedDay);
				this.secondSelectedDay = date;
				this.secondInput.value = "до " + this.getDateStringFromTimestamp(date);
			  }
			  this.selectOnClick();
			  this.calendar.innerHTML = "";
			  this.setCalBody();
		...
		setDateToInput(timestamp, input){
		  ...
		  if (input == this.firstInput) dateString = "от " + dateString;
		  if (input == this.secondInput) dateString = "до " + dateString;
